...
import static com.hexicube.solapple.lib.Localization.*;
...
final class ProgressGraph extends UIElement {
	...
	ProgressGraph(FoodData foodData, int centerX, int lineY) {
		...
		int previousMilestone = milestonesAchieved > 0 ? SOLAppleConfig.milestone(milestonesAchieved - 1) : 0;
		...
		UIImage appleIcon = new UIImage(FoodBookScreen.appleImage);
		appleIcon.setCenterY(lineY);
		if (milestonesAchieved > 0) {
			appleIcon.setMaxX(leftEdge - padding);
		} else {
			int distanceToLeftBookEdge = 36;
			appleIcon.setCenterX(leftPoint - distanceToLeftBookEdge / 2);
		}
		if (hasReachedMax) {
			appleIcon.tooltip = localized("gui", "food_book.stats.tooltip.progress.max");
		} else {
			appleIcon.tooltip = localized("gui", "food_book.stats.tooltip.progress", progressInfo.foodsUntilNextMilestone());
		}
		children.add(appleIcon);
		
		children.add(UIBox.horizontalLine(leftPoint, progressX, lineY, FoodBookScreen.fullBlack));
		
		UILabel previousMilestoneLabel = new UILabel(String.valueOf(previousMilestone));
		...
		UILabel foodsEatenLabel = new UILabel(String.valueOf(progressInfo.foodsEaten));
		...
		if (!hasReachedMax) {
			UILabel nextMilestoneLabel = new UILabel(String.valueOf(nextMilestone));
		...
		boolean isLastMilestoneVisible = milestonesAchieved + 1 >= SOLAppleConfig.getMilestoneCount();
	...
	private void addHeartsView(int centerX, int maxY, boolean isOpaque) {
		...
		int heartCount = SOLAppleConfig.getHeartsPerMilestone();
